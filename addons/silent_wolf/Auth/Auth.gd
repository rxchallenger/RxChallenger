...
func _on_LoginPlayer_request_completed(result, response_code, headers, body) -> void:
	...
	if status_check:
		var json_body = JSON.parse_string(body.get_string_from_utf8().strip_edges())
		...
		elif str(json_body.error) == "Could not log in: An error occurred (UserNotConfirmedException) when calling the InitiateAuth operation: User is not confirmed.":
